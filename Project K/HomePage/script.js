    ...
    if (numQuestions && numQuestions > 0) {
        ...
        for (let i = 1; i <= numQuestions; i++) {
            const questionBlock = `
                ...
                <input type="text" id="option4_${i}" name="option4_${i}" required><br><br>

                <label for="correct_${i}">Correct Option (1-4):</label>
                <input type="number" id="correct_${i}" name="correct_${i}" min="1" max="4" required><br><br>

                <label for="hint_${i}">Hint:</label>
                <input type="text" id="hint_${i}" name="hint_${i}" required><br><br>
            `;
...
function SaveTest() {
    ...
    h1.innerHTML = ("Saved Successfully");
    saveSuccess.appendChild("%c"+h1, ("color:blue;"));    
}
